...
public class WindowVisibilityHook extends BaseWindowHook {
    ...
    protected void onEvent(WinDef.DWORD event, WinDef.HWND hwnd,
        ...
        if (!windowToTrackChecker.isWindow(hwnd)) {
            logger.trace("[{}] ignored event={} for non tracked window={}", name(), event.intValue(), windowTitle);
            return;
        }

        // Ignore si la fenêtre est minimisée : on laisse MinimiseHook gérer ça
        if (window64Helper.isIconic(hwnd)) {
            logger.trace("[{}] ignored visibility event={} because window is iconic (hwnd={} title={})", name(), event.intValue(), hwnd, windowTitle);
            return;
        }
        // Déclencher SHOW ou HIDE
        if (event.intValue() == EVENT_OBJECT_SHOW) {
            logger.trace("[{}] tracked window show event for hwnd={} title={}", name(), hwnd, windowTitle);
//            dispatch(WindowEventAction.Show);
        } else if (event.intValue() == EVENT_OBJECT_HIDE) {
            logger.trace("[{}] tracked window hide event for hwnd={} title={}", name(), hwnd, windowTitle);
